...
- 모든 엔티티가 상속하는 공통 베이스. `id`(자동 생성)와 `equals`/`hashCode`(id 기반, 프록시 대응)를 제공한다.
...
- `updateInfo()`: 회원 정보(닉네임, 프로필 주소, 자기 소개)를 수정한다: `MemberInfoUpdateInfo`
- `isActive()`: 등록 완료 상태인지 확인한다.

#### 규칙

- 회원 등록 후에는 등록 대기 상태가 된다.
- 일정 조건을 만족하면 등록 완료 상태가 된다.
...
- `static create()`: 회원을 등록한다. 현재 시간을 등록 일시로 저장한다.
- `active()`: 등록 완료와 관련된 작업을 수행한다. 등록 완료 일시를 저장한다.
- `deactivate()`: 탈퇴와 관련된 작업을 수행한다. 탈퇴 일시를 저장한다.
- `updateInfo()`: 상세 정보(프로필 주소, 자기 소개)를 수정한다.
...
- `address`: 프로필 주소 (소문자/숫자, 15자 이내)

#### 행위

- `url()`: `@address` 형태의 프로필 URL을 반환한다.
...
- `Member.register()`에 전달되는 등록 정보: `email`, `nickname`, `password`
- 애플리케이션 계층의 `MemberRegisterRequest.toInfo()`로 변환되어 전달된다.

### MemberInfoUpdateInfo

_Value Object_

- `Member.updateInfo()`에 전달되는 수정 정보: `nickname`, `profileAddress`, `introduction`
- 애플리케이션 계층의 `MemberInfoUpdateRequest.toInfo()`로 변환되어 전달된다.

---

### Email

_Value Object (공통, domain.shared)_
